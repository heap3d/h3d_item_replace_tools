...
def is_bounding_box_intersects(mesh1, mesh2, search_dist, largest_rot, largest_pos):
    ...
    mesh1.select()
    lx.eval(
        "item.match item pos average:false item:{} itemTo:{}".format(bb1.id, mesh1.id)
    )
    lx.eval(
        "item.match item rot average:false item:{} itemTo:{}".format(bb1.id, mesh1.id)
    )
    lx.eval(
        "item.match item scl average:false item:{} itemTo:{}".format(bb1.id, mesh1.id)
    )
    bb2.select(replace=True)
    # mesh2_copy.select()
    mesh2.select()
    lx.eval(
        "item.match item pos average:false item:{} itemTo:{}".format(bb2.id, mesh2.id)
    )
    lx.eval(
        "item.match item rot average:false item:{} itemTo:{}".format(bb2.id, mesh2.id)
    )
    lx.eval(
        "item.match item scl average:false item:{} itemTo:{}".format(bb2.id, mesh2.id)
    )
...
def is_center_near(current_mesh, compare_mesh, search_dist):
    ...
    if (
        ...
        > cur_mesh_longest_length + comp_mesh_longest_length + search_dist * 2
    ):
        h3dd.print_fn_out()
        return False
...
def get_center_ratios(mesh):
    ...
    return mmu.Vector3(
        abs(v2.x * 2 / size.x), abs(v2.y * 2 / size.y), abs(v2.z * 2 / size.z)
    )
...
def is_valid_ratio(val1, val2, threshold):
    ...
    h3dd.print_debug(
        "val1 <{}>; val2 <{}>; threshold <{}>; val/val <{}>".format(
...
def is_equal_center_of_mass_pos(cur_mesh, cmp_mesh, threshold, options):
    ...
    cur_com_size = mmu.Vector3(
        cur_bb[1][0] - cur_com.x, cur_bb[1][1] - cur_com.y, cur_bb[1][2] - cur_com.z
    )
...
def is_mesh_similar(cur_mesh, cmp_mesh, options):
    ...
    # do checks
    h3dd.print_debug(
        "cur_mesh <{}>:<{}>; cmp_mesh <{}>:<{}>".format(
            cur_mesh.name, cur_mesh, cmp_mesh.name, cmp_mesh
        )
    )
    if any(options.do_bounding_box):
        if not is_similar_bounding_box(
            cur_mesh, cmp_mesh, options.bb_threshold, options
        ):
            h3dd.print_fn_out()
            return False
    if any(options.do_center_pos):
        if not is_similar_center_pos(
            cur_mesh, cmp_mesh, options.center_threshold, options
        ):
            h3dd.print_fn_out()
            return False
    if any(options.do_com_pos):
        if not is_similar_center_of_mass_pos(
            cur_mesh, cmp_mesh, options.com_threshold, options
        ):
            h3dd.print_fn_out()
            return False
...
def is_mesh_equal(cur_mesh, cmp_mesh, options):
    ...
    # do checks
    h3dd.print_debug(
        "Comparing bounding box of cur_mesh <{}> vs cmp_mesh <{}>   threshold <{}>...".format(
            cur_mesh.name, cmp_mesh.name, options.bb_threshold
        )
    )
    if any(options.do_bounding_box):
    ...
    if any(options.do_center_pos):
        if not is_equal_center_pos(
            ...
            return False
    if any(options.do_com_pos):
        if not is_equal_center_of_mass_pos(
            cur_mesh, cmp_mesh, options.com_threshold, options
        ):
...
def set_mesh_info(store_item, mesh_source):
    ...
    if mesh_source is None:
        ...
        return
    h3dd.print_debug(
        "store_ite {}>; mesh_source <{}>".format(store_item.name, mesh_source.name)
    )
...
def group_equal_meshes(meshes, options):
    ...
    h3dd.print_items(
        list(i.name for i in working_similar_groups),
        message="initial working_similar_groups:",
    )
    ...
    for similar_group in working_similar_groups:
        ...
        similar_group_num = name_sfx2num(
            similar_group.name, h3dc.GEO_SHAPE_SIMILAR_TYPE_NAME_BASE
        )
        type_nums = list(
            name_sfx2num(
                group.name,
...
def group_similar_items(meshes, options):
    ...
    h3dd.print_items(
        list(i.name for i in similar_groups), message="initial similar_groups:"
    )
    ...
    for mesh in meshes:
        ...
        h3dd.print_items(
            list(i.name for i in similar_groups), message="similar_groups:"
        )
        h3dd.print_debug("")
        ...
        new_similar_type_group = parent_item_to_item_name(
            mesh,
            "{}{}".format(
                h3dc.GEO_SHAPE_SIMILAR_TYPE_NAME_BASE, num2name_sfx(max_type_num)
            ),
        )
        # add new_similar_type_group to the similar_groups
        similar_groups.append(new_similar_type_group)
        h3dd.print_items(
            list(i.name for i in similar_groups),
...
def place_center_at_polygons(mesh, polys, largest_rot, largest_pos):
    ...
    parent = mesh.parent
    h3dd.print_debug(
        "mesh <{}>:<{}>; parent <{}>; polys <{}>".format(
            mesh.name, mesh, parent.name, polys
        )
    )
    mesh.select(replace=True)
...
def set_item_center(mesh, largest_rot, largest_pos):
    ...
    place_center_at_polygons(
        mesh=mesh, polys=polys, largest_rot=largest_rot, largest_pos=largest_pos
    )
...
def set_item_center_normalized(mesh, largest_rot, largest_pos, threshold):
    ...
    h3dd.print_debug(
        "largest_poly index <{}>; rel_area_percent <{}>; area <{}>; mesh full area <{}>".format(
            largest_poly.index,
            rel_area_percent,
            largest_poly.area,
            h3du.get_full_mesh_area(mesh),
        )
    )
